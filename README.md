...
**核心目标**：通过标签聚合写入、双磁头调度与垃圾回收，将随机读转化为顺序读，降低碎片化，提升整体读取性能。

**赛题硬约束**：

- 每个对象必须恰好 **3 个副本**，同一副本的对象块写在同一块盘，不同副本写不同盘
- 每块盘 **V 个存储单元**（环形排列），**2 个磁头**，每磁头每时间片最多消耗 **G 个令牌**
- 每个读请求必须在 **105 个时间片内**返回成功、繁忙或被删除取消
- 决赛中 **对象标签可能隐藏**（`tag = 0`），需根据用户行为反向推理
...
`BFSHeadSchedule` 是基于优先队列的 BFS 搜索调度器，作为对比方案保留，实际未启用。
