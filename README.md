...
| 第二轮 | `Disk` | 加权平方误差全局预测 | 利用预测标签优化布局|
...
三副本分别写入不同盘。主存储区进一步划分为多个**区域**（`field`），每个域绑定一个标签，域内对象顺序存储。读取时磁头只需在域内有收益的区间内顺序扫描，读完后才 Jump 到新域。

### 2.3 写入布局

写入策略分两层：

**磁盘级**：基于「盘空间余量 + 请求频率」选择三块目标盘，平衡硬盘压力。

**区域级**：使用 `WriteTool::feature_storage` 按对象生命周期特征选择落盘位置。核心逻辑是：
