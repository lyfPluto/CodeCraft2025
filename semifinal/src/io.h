...
void pretreatment() {
    ...
    scanf("%d%d%d%d%d%d", &GlobalVariable::T, &GlobalVariable::M, &GlobalVariable::N, 
        &GlobalVariable::V, &GlobalVariable::G, &GlobalVariable::K);
    /**
    ...
    for (int i = 1; i <= GlobalVariable::M; i++) {
        for (int j = 1; j <= (GlobalVariable::T - 1) / FRE_PER_SLICING + 1; j++) {
            int input;
            scanf("%d", &input);
            GlobalVariable::delete_frequency[i].push_back(input);
        }
    }
    ...
    for (int i = 1; i <= GlobalVariable::M; i++) {
        for (int j = 1; j <= (GlobalVariable::T - 1) / FRE_PER_SLICING + 1; j++) {
            int input;
            scanf("%d", &input);
            GlobalVariable::write_frequency[i].push_back(input);
        }
    }
    ...
    for (int i = 1; i <= GlobalVariable::M; i++) {
        for (int j = 1; j <= (GlobalVariable::T - 1) / FRE_PER_SLICING + 1; j++) {
            int input;
            scanf("%d", &input);
            GlobalVariable::read_frequency[i].push_back(input);
        }
    }
...
void read_print() {
    read_out.output();
    read_out = read_action_output(GlobalVariable::N);
    printf("%d\n", read_complete.size());
