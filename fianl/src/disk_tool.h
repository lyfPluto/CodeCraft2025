...
public:
    ...
    static std::pair<std::vector<std::vector<field_info>>, std::vector<field_info>> field_distribution(
        ...
        for (int i = 1; i <= tag_num; i ++) {
            tag_storage[i - 1] = 10000;

        }
        long long sum = 0;
        for (int num: tag_storage) sum += num;
        double discount = (((double) GlobalInfo::N * GlobalInfo::V) / (3 * sum));
        discount *= 1.08;
        for (int& num: tag_storage) num *= discount;
        int seg_count = GlobalInfo::N * 9;
        int min_len = 0, max_len = 1000000;
        while (min_len < max_len - 1) {
            int mid_len = (min_len + max_len) / 2;
            if (mid_len == 0) {
                throw Exception("mid_len值异常");
            }
            int seg_need = 0;
            for (int i = 0; i < GlobalInfo::M; i ++) {
        ...
        for (int i = 0; i < tag_num; i ++) {
            ...
            for (int j = 0; j < seg_tag; j ++) {
                ...
                {
                    disk_index2 = -1;
                    for (int candicate_id = 0; candicate_id < GlobalInfo::N; ++ candicate_id) {
                        if (disk_candidate[candicate_id].second == (disk_candidate[disk_index1].second + 1) % GlobalInfo::N) {
    ...
    // 磁盘区域划分
    static std::pair<std::vector<std::vector<field_info>>, std::vector<field_info>> secondry_field_distribution(
        int disk_num, int disk_size, int tag_num) {
        debug << "start field_distribution" << newLine;
        std::vector<std::vector<field_info>> field_group;
        std::vector<field_info> backup_field;
        ...
        for (int obj_id = 1; obj_id < GlobalInfo::obj_tags.size(); ++ obj_id) {
            tag_storage[GlobalInfo::obj_tags[obj_id] - 1] += GlobalInfo::obj_size[obj_id] * 
                (GlobalInfo::obj_delete_time[obj_id] - GlobalInfo::obj_create_time[obj_id]);
        }
        long long sum = 0;
        for (int num: tag_storage) sum += num;
        double discount = (((double)GlobalInfo::N * GlobalInfo::V) / (3 * sum));
