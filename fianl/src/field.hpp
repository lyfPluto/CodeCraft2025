...
void field::save_separate(ObjectBase* obj) {
    disk -> save_separate(obj, this);
};
void field::save(ObjectBase* obj) {
    disk -> save(obj, this);
}
void field::try_save(ObjectBase* obj) {
    disk -> try_save(obj, this);
};
int field::get_end()  {
    return (start + size) % ( disk -> size);
}
int field::get_benefit_begin() const {
    EXCEPTION_CHECK(benefit_index.empty(), "field::get_benefit_begin: benefit_index为空，无法获取起始位置。");
    return (start + *benefit_index.begin() + benefit_index_delta) % disk -> size;
}
int field::get_benefit_end() const {
    EXCEPTION_CHECK(benefit_index.empty(), "field::get_benefit_end: benefit_index为空，无法获取起始位置。");
